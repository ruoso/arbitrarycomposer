...
constexpr std::array<unsigned char, 32> kFillExp32{{0xCD, 0xCC, 0xCC, 0x3D, 0xCD, 0xCC, 0x4C, 0x3E, 0x9A, 0x99, 0x99, 0x3E, 0x00, 0x00, 0x00, 0x3F, 0xCD, 0xCC, 0xCC, 0x3D, 0xCD, 0xCC, 0x4C, 0x3E, 0x9A, 0x99, 0x99, 0x3E, 0x00, 0x00, 0x00, 0x3F}};
constexpr std::array<unsigned char, 16> kFillExp16{{0x66, 0x2E, 0x66, 0x32, 0xCD, 0x34, 0x00, 0x38, 0x66, 0x2E, 0x66, 0x32, 0xCD, 0x34, 0x00, 0x38}};
...
constexpr std::array<unsigned char, 64> kSoIntExp32{{0xCD, 0xCC, 0xCC, 0x3E, 0x34, 0x33, 0xB3, 0x3E, 0x9A, 0x99, 0x19, 0x3E, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x80, 0x3E, 0x33, 0x33, 0xB3, 0x3E, 0x9A, 0x99, 0x99, 0x3E, 0x00, 0x00, 0x80, 0x3F, 0xCD, 0xCC, 0xCC, 0x3D, 0x00, 0x00, 0x80, 0x3E, 0xCD, 0xCC, 0x0C, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x66, 0x66, 0xE6, 0x3E, 0x9A, 0x99, 0x99, 0x3E, 0xCD, 0xCC, 0x4C, 0x3E, 0x00, 0x00, 0x80, 0x3F}};
constexpr std::array<unsigned char, 32> kSoIntExp16{{0x66, 0x36, 0x9A, 0x35, 0xCC, 0x30, 0x00, 0x3C, 0x00, 0x34, 0x9A, 0x35, 0xCD, 0x34, 0x00, 0x3C, 0x66, 0x2E, 0x00, 0x34, 0x66, 0x38, 0x00, 0x3C, 0x33, 0x37, 0xCC, 0x34, 0x66, 0x32, 0x00, 0x3C}};
constexpr std::array<unsigned char, 16> kSoIntExp8{{0xAA, 0xA0, 0x6C, 0xFF, 0x89, 0xA0, 0x95, 0xFF, 0x59, 0x89, 0xC4, 0xFF, 0xB3, 0x95, 0x7C, 0xFF}};

// source_over_kernel fractional (translation 0.5,0), 2x2, per format.
constexpr std::array<unsigned char, 64> kSoFracExp32{{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x3F}};
constexpr std::array<unsigned char, 32> kSoFracExp16{{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0x00, 0x38, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0x00, 0x38, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3C}};
constexpr std::array<unsigned char, 16> kSoFracExp8{{0x00, 0x00, 0x00, 0x80, 0xBC, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x80, 0xBC, 0x00, 0x00, 0xFF}};

// downsample_box_kernel 2x2 -> 1x1, per format.
constexpr std::array<unsigned char, 16> kDownExp32{{0x66, 0x66, 0xE6, 0x3E, 0x67, 0x66, 0xA6, 0x3E, 0x9A, 0x99, 0xD9, 0x3E, 0x00, 0x00, 0x80, 0x3F}};
...
constexpr std::array<unsigned char, 16> kConv_32_16{{0x66, 0x32, 0x66, 0x36, 0xCD, 0x38, 0x00, 0x3C, 0xCD, 0x34, 0xCD, 0x30, 0x66, 0x2A, 0xCD, 0x38}};
constexpr std::array<unsigned char, 8> kConv_32_8{{0x7C, 0xAA, 0xCB, 0xFF, 0xBC, 0x89, 0x52, 0x99}};
constexpr std::array<unsigned char, 32> kConv_16_32{{0x00, 0xC0, 0x4C, 0x3E, 0x00, 0xC0, 0xCC, 0x3E, 0x00, 0xA0, 0x19, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x00, 0xA0, 0x99, 0x3E, 0x00, 0xA0, 0x19, 0x3E, 0x00, 0xC0, 0x4C, 0x3D, 0x00, 0xA0, 0x19, 0x3F}};
constexpr std::array<unsigned char, 8> kConv_16_8{{0x7C, 0xAA, 0xCB, 0xFF, 0xBC, 0x89, 0x51, 0x99}};
constexpr std::array<unsigned char, 32> kConv_8_32{{0xC2, 0x64, 0x4E, 0x3E, 0x0B, 0xD0, 0xCD, 0x3E, 0x39, 0xE2, 0x18, 0x3F, 0x00, 0x00, 0x80, 0x3F, 0x9D, 0x7C, 0x9A, 0x3E, 0x83, 0xB2, 0x19, 0x3E, 0xED, 0x5C, 0x4F, 0x3D, 0x9A, 0x99, 0x19, 0x3F}};
constexpr std::array<unsigned char, 16> kConv_8_16{{0x73, 0x32, 0x6F, 0x36, 0xC7, 0x38, 0x00, 0x3C, 0xD4, 0x34, 0xCE, 0x30, 0x7B, 0x2A, 0xCD, 0x38}};

// Codec reference vectors (exact bit patterns / codes).
constexpr std::array<std::uint32_t, 4> kSrgb8ToLinear{{0x0, 0x399F22B4, 0x3E5D0A8B, 0x3F800000}}; // codes 0,1,128,255
constexpr std::array<unsigned char, 4> kLinearToSrgb8{{0x0, 0x8, 0xBC, 0xFF}}; // inputs 0.0,0.0025,0.5,1.0
constexpr std::array<unsigned char, 3> kUnorm8Encode{{0x0, 0x80, 0xFF}};       // inputs 0.0,0.5,1.0
constexpr std::array<std::uint32_t, 3> kUnorm8Decode{{0x0, 0x3F008081, 0x3F800000}}; // codes 0,128,255
// halves 0x0000,0x8000(-0),0x3800(0.5),0x3C00(1.0),0x0001(subnormal),0x7C00(inf)
constexpr std::array<std::uint32_t, 6> kF16ToFloat{{0x0, 0x80000000, 0x3F000000, 0x3F800000, 0x33800000, 0x7F800000}};
...
constexpr std::array<std::uint32_t, 4> kPremultiply{{0x3EF5C290, 0x3E75C290, 0x3DF5C290, 0x3F19999A}};   // straight {0.8,0.4,0.2,0.6}
constexpr std::array<std::uint32_t, 4> kUnpremultiply{{0x3F4CCCCC, 0x3ECCCCCC, 0x3E4CCCCC, 0x3F19999A}}; // premul  {0.48,0.24,0.12,0.6}
...
TEST_CASE("source_over_kernel fractional bilinear tap is byte-exact per format") {
  require_bytes(src_over_fractional_bytes<PixelFormat::Rgba32fLinearPremul>(arbc::k_working_rgba32f),
                kSoFracExp32);
  require_bytes(src_over_fractional_bytes<PixelFormat::Rgba16fLinearPremul>(arbc::k_working_rgba16f),
                kSoFracExp16);
...
TEST_CASE("software f16 codec pins its absolute bytes at the IEEE landmarks") {
  const std::array<std::uint16_t, 6> halves{0x0000U, 0x8000U, 0x3800U,
                                            0x3C00U, 0x0001U, 0x7C00U};
...
void dump_bytes(const char* name, std::size_t n, const std::vector<std::byte>& b) {
  ...
  for (std::size_t i = 0; i < b.size(); ++i) {
    std::printf("%s0x%02X", i ? ", " : "", static_cast<unsigned>(std::to_integer<unsigned char>(b[i])));
  }
  std::printf("}};\n");
}
...
TEST_CASE("regenerate frozen kernel/codec tables", "[.regen]") {
  dump_bytes("kFillExp32", 0, fill_bytes<PixelFormat::Rgba32fLinearPremul>(arbc::k_working_rgba32f));
  dump_bytes("kFillExp16", 0, fill_bytes<PixelFormat::Rgba16fLinearPremul>(arbc::k_working_rgba16f));
