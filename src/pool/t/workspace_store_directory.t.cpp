...
arbc::WorkspaceStoreEntry read_store_row(const std::string& path, const arbc::WorkspaceHeader& hdr,
  ...
  const std::vector<std::uint8_t> raw =
      read_bytes(path, offset, sizeof(arbc::WorkspaceStoreEntry));
...
TEST_CASE("a reopening build whose geometry disagrees with the store table is refused") {
  ...
  SECTION("a high-water the store's chunks do not cover -> StoreDirectoryInconsistent") {
    ...
    write_at(dst.str(),
             hdr->store_table_offset + offsetof(arbc::WorkspaceStoreEntry, high_water),
  ...
  SECTION("a chunk tagged with a row the table cannot hold -> StoreDirectoryInconsistent") {
    ...
    write_at(dst.str(),
             sizeof(arbc::WorkspaceHeader) + offsetof(arbc::WorkspaceChunkEntry, owner),
  ...
  SECTION("a non-power-of-two slots-per-chunk -> StoreLayoutMismatch, never a bad shift") {
    ...
    write_at(dst.str(),
             hdr->store_table_offset + offsetof(arbc::WorkspaceStoreEntry, chunk_slots),
  ...
  SECTION("a non-canonical slot alignment -> StoreLayoutMismatch, never a duplicate store") {
    ...
    write_at(dst.str(),
             hdr->store_table_offset + offsetof(arbc::WorkspaceStoreEntry, slot_align),
...
TEST_CASE("the default layout's store tables cost zero extra pages") {
  ...
  const std::uint64_t format1_data_offset = arbc::align_up(
      sizeof(arbc::WorkspaceHeader) + std::size_t{arbc::k_workspace_default_max_chunks} *
  ...
  REQUIRE(hdr->store_table_offset ==
          sizeof(arbc::WorkspaceHeader) + std::uint64_t{arbc::k_workspace_default_max_chunks} *
                                              sizeof(arbc::WorkspaceChunkEntry));
  REQUIRE(hdr->store_table_offset +
              2u * hdr->max_stores * sizeof(arbc::WorkspaceStoreEntry) <= hdr->data_offset);
}
...
TEST_CASE("commit publishes every registered store's high-water with zero extra syscalls",
  ...
  public:
    int before(arbc::WorkspaceSyscall kind, std::uint64_t file_offset, std::size_t) noexcept
        override {
...
constexpr std::size_t k_golden_bytes =
    ...
    k_golden_max_chunks * sizeof(arbc::WorkspaceChunkEntry) +
...
constexpr std::uint8_t k_golden_header_region[k_golden_bytes] = {
    0x41, 0x52, 0x42, 0x43, 0x57, 0x53, 0x42, 0x46, 0x02, 0x00, 0x00, 0x00,
    ...
    ,
};
