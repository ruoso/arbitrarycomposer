...
constexpr std::array<unsigned char, 128> k_tone48k = {
...
constexpr std::array<unsigned char, 128> k_tone44k = {
...
TEST_CASE("org.arbc.tone renders a byte-exact golden at 48 kHz") {
  require_bytes(render_tone_bytes(48000), k_tone48k);
}

// enforces: 12-audio#tone-renders-at-any-requested-rate
TEST_CASE("org.arbc.tone renders a byte-exact golden at 44100 Hz") {
  require_bytes(render_tone_bytes(44100), k_tone44k);
}
...
TEST_CASE("dump tone goldens", "[.regen]") {
  dump("k_tone48k", render_tone_bytes(48000));
  dump("k_tone44k", render_tone_bytes(44100));
}
// GCOV_EXCL_STOP
