...
- **Updated** (2026-07-25, triage against the one-render-path unification): **the entry's CPU half is now moot, and its GPU half is the whole entry.** `compositor.render_path_unification` retired the untiled `render_frame`, which was the *only* place a provided surface was ever composited without a copy — the tiled driver renders each miss straight into the cache-owned surface it will insert and copies a provided surface into that same surface. The one surviving caller of the inline composite, `render_frame_anchored`, has no production driver behind it. So there is no zero-copy consumer left to optimize *around*: cache adoption is now the only place a copy could be avoided at all, which is exactly what this entry proposes. Doc 09's realization addendum 2 was amended in the same commit to say so. Still parked on the same gate — a GPU backend making the copy a measured cost — but the alternative that kept deferring it ("shrink the copy instead of eliminating it") is gone: the copies are already tile-sized.
