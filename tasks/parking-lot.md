...
### 2026-07-12 — `compositor.refine_frame_composite_idempotence` blocks non-zero interactive worker default
- **Source**: closer for `runtime.interactive_worker_count_default` (re-defer commit); implementer return summary.
- **Question**: A damage-gated refine frame re-composites onto the caller-persisted target without clearing first, so source-over lands twice. 2/60 runs with `worker_count=1` under CPU oversubscription on a nested semi-transparent scene produce `composites=6` where the single-pass oracle does `composites=3`; the loop quiesces cleanly on wrong pixels. The WBS task `compositor.refine_frame_composite_idempotence` covers the fix. Should v0.1 carry the fix before `runtime.interactive_worker_count_default` ships, or should the default remain 0 at release and the fix land as a follow-up? The current WBS graph expresses the fix as a prerequisite (M9 already depends on both).
- **Why parked**: This is a scope/quality-bar judgment: the fix is a compositor correctness bug that only fires on the interactive refine path with `worker_count > 0`, which v0.1 does not yet enable. The decision whether that constitutes a release blocker or a post-ship patch depends on the v0.1 quality bar and ship timeline. Human call once `compositor.refine_frame_composite_idempotence` is scoped and the implementation effort is clear.
