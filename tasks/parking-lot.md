...
- **Resolved** (2026-07-10, triage): amend-the-docs chosen over redesigning the HAMT (the reclaim cascade's child-edge release depends on the non-trivial destructor; `workspace_backing` Decision 6 already established file backing is unaffected). Docs 14/15 now distinguish trivially-destructible *object records* from the persistent map's *interior nodes* with the sink-routed destructor cascade.
...
- **Resolved** (2026-07-10, triage): doc 13 amended to carry the explicit covering/over-approximation wording at its `map_input_damage` bullet, retiring the ambiguity — the header comment is now a restatement of normative doc text.
...
- **Resolved** (2026-07-10, triage): the trigger has fired — `model.editable_facet` and the slab-arena migration landed (Model owns the arenas, sinks, and reclaim context; the old `document.hpp` migration comment is gone) but the Housekeeper still has no live-document consumer. Scoped as `runtime.housekeeping_document_wiring` (tasks/65-runtime.tji), depending on `model.workspace_backing` (which brings the Checkpointer to Model); wired to M9.
...
- **Resolved** (2026-07-10, triage): doc 14's transaction example now enumerates `set_opacity` alongside `set_span`/`set_transform` as a core placement mutator (all auto-damaging). No downstream task is obligated to call it — it is ordinary model API surface for hosts; no retroactive refinement update needed.
...
- **Resolved** (2026-07-10, triage): moot — `contract.audio_facet` landed and `PullServiceImpl::pull_audio` is fully implemented (`src/compositor/pull_service.cpp:383`); the audio pull joined the `PullService` interface as anticipated. No leaf needed.
...
- **Resolved** (2026-07-10, triage): confirmed a genuine Principle 4 violation — the principle's wording is unqualified and doc 08's own example layer field (`"name"`, doc 08:35) is dropped today; the same gap exists at composition, document, and content-body-sibling tiers. Fix-the-loader chosen over narrowing the doc: scoped as `serialize.unknown_field_preservation` (tasks/60-serialize.tji), wired to M8 — preserve at every tier via an opaque serialized-bytes stash (keeps the JSON type private to `arbc::serialize`). Doc 08 Principle 4 amended to state the every-tier scope explicitly.
...
- **Resolved** (2026-07-10, triage): option (a) chosen — in-document `compositions` table keyed by core-owned non-semantic ids, with a core-owned `composition` reference field on nested content bodies; settles doc 05 Droste serialization (cycles round-trip as data). Docs 00/05/08 amended (doc 08 Principle 7). Implementation scoped as `serialize.compositions_table` + `runtime.nested_codec`, both wired to M8. External `params.ref` remains the external-project form.
...
- **Source**: re-defer for `model.workspace_backing` (nothing landed — no commit).
- **Question**: How should the workspace file format record per-store chunk ownership and per-store high-water so that `Model::open` can partition reopened chunks between `d_nodes` (HamtNode) and `d_records` (ObjectRecord) during cold recovery? The implementer identified three candidate resolutions: **(A)** per-store chunk directory in the workspace format (`pool` + format work; realizes doc 15:199-203 "per-type slot sizes, and arena directory" — that field was never implemented; requires a `pool` refinement first, then this task consumes it); **(B)** size/owner-routed reopen `acquire` (match chunks to the requesting store's `d_chunk_bytes` — fragile, and fails in the ASan/debug lane where both `HamtNode` and `ObjectRecord` strides yield 36864-byte chunks); **(C)** one store per file, two `Checkpointer`s, dependency-ordered commit (records file durable before nodes file — fully correct with current pool seams, but contradicts Decision 1 / doc 15's "per-document workspace file" singular and requires a design-doc delta overriding Decisions 1 and 7). Of these, A is the doc-faithful fix but is genuine pool+format work; C is the cleanest correct realization with current seams but deviates from the stated design.
- **Why parked**: Each candidate requires a human design decision before a follow-up WBS task can be properly scoped and wired to M8. A/B require a new `pool`-layer seam (chunk-directory API); C requires a doc 15 delta and an explicit override of Decisions 1 and 7 in the refinement. No agent can make this call unilaterally. Once the direction is chosen, the follow-up WBS task (tentatively `pool.workspace_store_directory` for resolution A, or a model-only format-deviation leaf for resolution C) can be scoped and added to M8's `depends` list in place of `model.workspace_backing`.
- **Resolved** (2026-07-10, triage): resolution (A) chosen — per-store chunk directory in the workspace format, realizing the doc 15 "per-type slot sizes, and arena directory" header field that was never implemented (doc-faithful, no doc delta). Scoped as `pool.workspace_store_directory` (tasks/05-pool.tji); `model.workspace_backing` now depends on it and remains the M8 gate. B rejected (chunk byte-size is ambiguous in the debug lane); C rejected (doc 15 delta overriding refinement Decisions 1 and 7).

### 2026-07-09 — `operators.fade_runtime_binding` re-deferred: visual pull delivery gap in PullServiceImpl

- **Source**: re-defer for `operators.fade_runtime_binding` (see accompanying commit; nothing landed).
- **Question**: No agent decision needed — `compositor.operator_pull_delivers_target` has been registered as the concrete predecessor WBS task (`tasks/35-compositor.tji`). This entry is the re-pick trigger pointer.
- **Why parked**: Preconditions unmet. `PullServiceImpl::pull` does not deliver pulled pixels to the caller's `request.target`; it renders into a private cache tile and settles with metadata only. Wiring `FadeContent` to the live service produces transparent visual output (byte-exact golden unachievable) and a use-after-free SIGSEGV on the parallel path (nested input pull dispatched with `pending=nullptr`). The fix is a compositor-contract change that must happen before any operator runtime-binding task can succeed (`operators.fade_runtime_binding`, `operators.crossfade_runtime_binding`, `kinds.nested_runtime_binding` all share the same blocker). Re-pick `operators.fade_runtime_binding` once `compositor.operator_pull_delivers_target` is `complete 100`.
- **Resolved** (2026-07-10, triage): the re-pick happened — `compositor.operator_pull_delivers_target` is `complete 100`, and `operators.fade_runtime_binding` and `operators.crossfade_runtime_binding` both landed. Loop closed.

### 2026-07-10 — Interactive audio `PullConfig::id_of` wiring (no production `id_of` on the interactive audio pump)

- **Source**: closer for `runtime.operator_input_cache_identity`; flagged in refinement Open questions.
- **Question**: The interactive audio pump (`lookahead_pump.cpp` / `device_monitor.cpp`) builds no production `PullConfig::id_of` today — all content including layer roots keys on `ObjectId{}`. This is a broader "interactive audio not yet fully pull-wired" gap, not a multi-input collision. Should the interactive-audio wiring stream scope a leaf to reuse the new `pull_identity` helper once interactive audio is fully pull-wired? Or does an existing interactive-audio task already cover it?
...
- **Question**: `interactive.cpp:115` calls `render_frame_interactive` with `pulls == nullptr` and builds no production `PullConfig::id_of`, so the identity-delivery branch added by `runtime.operator_identity_offline_delivery` does not fire on the interactive path — an interactive operator layer at its endpoint (`w==0`/`w==1`) would still render blank. Does an existing interactive-stream task already cover passing a `PullServiceImpl` (with `id_of`) into `render_frame_interactive`? If so, that task will inherit this fix for free once it wires `pulls`; if not, a new leaf should be scoped and wired to the appropriate milestone.
- **Why parked**: Minting a leaf now risks duplicating an existing interactive-stream task. The fix machinery is fully landed — the interactive path inherits it the moment `pulls` is non-null with a proper `id_of`. Human call to audit the existing interactive-stream task list and confirm coverage or scope the missing leaf.
- **Resolved** (2026-07-10, triage): audited — no existing task passes `pulls` into `render_frame_interactive`. Scoped as `runtime.interactive_pull_wiring` (tasks/65-runtime.tji); wired to M9.

### 2026-07-10 — Editable-content removal path: trigger and model-transaction semantics

- **Source**: closer for `kinds.raster_runtime_binding`; surfaced in refinement Open questions section.
- **Question**: Does v1 need a per-content removal path (e.g. `Document::remove_content`)? The current teardown is document-wide (`~Document`); the `EditableBinding::unbind()` method is structured to accept per-content teardown once a removal API exists. The trigger (when a content should be removable) and the model-transaction semantics of removal (what happens to journal entries referencing the removed content) are product/design judgments.
- **Why parked**: Agent-decidable work (the unbind method) is already in place. The open question is whether and when removal should be surfaced as a user-facing operation and what the undo/redo contract is for removed content — design and product calls that must precede any WBS task.

### 2026-07-11 — `compositor.root_composition_frame_walk` double-draw of nested layers
- **Source**: closer for `kinds.nested_runtime_binding`; flagged in fixer attempt 1 tech-debt.
- **Question**: `render_frame` / `tile_planning`'s visual frame walk is unconditionally global (`for_each_layer`) and ignores composition membership, unlike `ExportMonitor`'s composition-scoped audio mix (`for_each_layer_in`). A document that nests a child composition therefore draws that child's layers twice: once at top level, once through nested — so a bound `NestedContent` renders a double-draw in the product frame even though `NestedContent::render` itself is correct. Should the visual frame walk gain a "which composition am I rendering" input so it scopes to only the root composition's layers, letting nested's own render handle the child's layers?
- **Why parked**: The bug is pre-existing and architectural — the driver has no composition-membership input today. Fixing it requires a compositor-driver change (`render_frame`/`tile_planning` signatures or a new walk variant) and possibly a design-doc delta. The scope and priority are a human judgment call; the task depends on what shape the scoped visual walk takes. Not an agent-implementable leaf until the compositor walk design is settled.

### 2026-07-11 — `arbc::media` audio coefficient table has no live `[.regen]` path
- **Source**: closer for `kinds.raster_resampling_quality`; flagged in refinement Open questions and implementer return summary.
- **Question**: `src/media/audio_resampler.cpp:31-41` documents a REGENERATE PROCEDURE via the `[.regen]` case, but the only `[.regen]` case in `arbc_media_t` dumps the *output* golden (`t/audio_resampler.t.cpp:184`), not the 512-float coefficient bank. `kinds.raster_resampling_quality` closed the equivalent gap for the image resampler (its `[.regen]` dumps the coefficient table itself). Should a two-line dumper be added to `src/media/t/audio_resampler.t.cpp` to regenerate the audio bank as well?
- **Why parked**: The fix is trivial (mirroring the image resampler's `[.regen]` structure) but it touches audio golden infrastructure — a human should decide whether to bundle it with the next audio task or do it standalone. Not a WBS leaf (the work is ~2 lines, not a day's effort); whoever next opens `audio_resampler.t.cpp` should just make it.
...
### 2026-07-11 — `pool.header_writeback_ordering`: kernel write-ordering gap between root-slot and store-table pages
- **Source**: closer for `pool.workspace_store_directory`; flagged by implementer as a pre-existing property not changed.
- **Question**: `commit`'s "zero extra msyncs" (Decision 4 of the refinement) rests on the premise that no header page reaches disk before `sync_header()`. The kernel gives no ordering guarantee between the root-slot page and the store-table page, so an early kernel writeback of the root could pair a *new* root with a *stale* store-table snapshot, leaving `adopt_snapshot` punching chunks the root needs. Closing this exposure requires an extra msync between the store-table write and the root-slot flip — which the refinement's Decision 4 and its behavioral-counter acceptance criterion explicitly forbid. Should a future task add that msync (changing the acceptance criterion), accept the risk as inherent to the A/B protocol, or pursue a layout approach that collapses root slot and store table onto the same page?
- **Why parked**: The exposure is pre-existing (the chunk directory has the identical ordering gap in the landed A/B protocol). Changing the syscall count requires amending the behavioral-counter acceptance criterion (Decision 4 / doc 16:66-78 counter tests) — a scope and priority judgment. Not agent-implementable without a human decision on whether the msync cost is acceptable.

### 2026-07-11 — `pool.clean_close_preserves_file`: clean teardown hole-punches all live chunks
- **Source**: closer for `pool.workspace_store_directory`; flagged by implementer as a pre-existing `mmap_backing` release behavior that `model.workspace_backing` will encounter.
- **Question**: A clean `WorkspaceFile` teardown (non-crash) still calls the fenced hole-punch release path on every live chunk (`mmap_backing` release semantics), so only a *crashed* writer leaves a reopenable workspace file — a clean save-and-close then reopen regenerates from JSON. Should `mmap_backing`'s release semantics be changed so a clean teardown preserves the file, or should `model.workspace_backing` add a "preserve on clean close" mode that skips the hole-punch? The current behavior is safe but makes the workspace cache useless across clean restarts.
- **Why parked**: Changing `mmap_backing` release semantics affects every `ChunkSource` consumer. Whether to fix this in pool or model, and at what priority, is a design judgment. `model.workspace_backing` will need to document its behavior around this when it lands — human call on the right fix point.
...
### 2026-07-11 — `pool.checkpoints_mprotect_reopen`: Checkpointer::commit mprotect re-seals free-list pages
- **Source**: closer for `model.workspace_backing`; flagged by implementer as a pre-existing pool.checkpoints defect fixed as a side-effect.
- **Question**: `Checkpointer::commit`'s debug seal re-mprotects whole chunks read-only after `drain_fences`, including pages holding free-list slots freed in an earlier epoch (only the current epoch's quarantine pages were reopened). The first allocation into a recycled slot hits `SEGV_ACCERR`. The fix (reopening all reusable slots' pages after the drain) is in the landed change. Should this fix get its own regression test in `pool.checkpoints` tests, or is the coverage from `model.workspace_backing`'s asan lane sufficient?
- **Why parked**: The bug is fixed (landed in `src/pool/arbc/pool/checkpoint.hpp` + `slot_store.cpp`); the question is whether a pool-layer regression test should be added to prevent re-regression without needing a full model test. Coverage judgment — human call.
