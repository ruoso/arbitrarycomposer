...
### 2026-07-16 — Push v0.1.0 tag to remote and create GitHub release
- **Source**: closer for `packaging.release_01` (this commit); doc 10 § Cutting a release step 5; `packaging.tag_01` WBS note.
- **Question**: After `packaging.tag_01` creates the annotated `v0.1.0` tag locally, the tag must be pushed (`git push origin v0.1.0`) and any GitHub release or publication artifact created. Should a GitHub release be created, and what assets (source tarball, CPack TGZ) should be attached?
- **Why parked**: Publishing and release-asset decisions require human action — the WBS cannot push to a remote or authorize publication. `packaging.tag_01` is the last WBS leaf; once it closes, this entry is the trigger for the human to push. The doc 10 ritual already documents "pushing the tag and any publication is a human step."
- **Updated** (2026-07-17, closer for `packaging.tag_01`): **the trigger has fired.** `packaging.tag_01` is `complete 100` and **M9 (`m9_release`) is closed at 141/141**. The annotated `v0.1.0` tag now exists **locally** on commit `2ead58e` (the changelog roll), its message the rolled `[0.1.0]` CHANGELOG section verbatim (doc 10 step 4). What remains is exactly and only the human step: `git push origin v0.1.0`, and the decision of whether to create a GitHub release and which assets to attach (source tarball / CPack TGZ). Still parked — this is the live action item now, not a future one.
